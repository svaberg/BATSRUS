...
module ModVarIndexes
  ...
  character (len=*), parameter :: &
       NameEquationFile = "ModEquationMhdHpH.f90"

  ! Standard single-fluid MHD state with two species densities:
  ! proton (Hp) and neutral hydrogen (H).
  character (len=*), parameter :: &
       NameEquation = 'MHD 2 Species (Hp+H), Earth XUV'
  ...
  ! Two species densities carried as scalars.
  integer, parameter :: ScalarFirst_ = RhoHp_, ScalarLast_ = RhoH_

  ! Species
  integer, parameter :: SpeciesFirst_   = ScalarFirst_
  integer, parameter :: SpeciesLast_    = ScalarLast_

  ! Molecular masses in AMU for Hp and H.
